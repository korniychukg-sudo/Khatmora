...
struct PaceRing: View {
    ...
    var tint: Color = QPTheme.indigo
    var track: Color = QPTheme.line.opacity(0.5)
    var body: some View {
        ZStack {
...
struct QPBadgeToast: View {
    let badge: QPBadge
    var body: some View {
        HStack(spacing: 12) {
            ZStack {
                Circle().fill(QPTheme.gold.opacity(0.18)).frame(width: 42, height: 42)
                OctoStar(points: 8)
                    .fill(QPTheme.gold)
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(QPTheme.card)
                .shadow(color: QPTheme.ink.opacity(0.18), radius: 14, x: 0, y: 6)
        )
...
struct QPArrow: View {
    var size: CGFloat = 12
    var color: Color = QPTheme.inkFaint
    var body: some View {
        Canvas { ctx, sz in
            var p = Path()
