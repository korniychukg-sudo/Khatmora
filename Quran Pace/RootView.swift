...
struct SealCelebration: View {
    ...
    var body: some View {
        ZStack {
            QPTheme.ink.opacity(0.35).ignoresSafeArea()
            VStack(spacing: 14) {
                ZStack {
                    MedallionRosette(tint: QPTheme.gold, petals: 10)
                        .frame(width: 104, height: 104)
                    OctoStar(points: 8)
                        .fill(QPTheme.gold)
                        .frame(width: 40, height: 40)
                    Text("\(juz)")
                        .font(QPTheme.round(15))
                        .foregroundColor(.white)
                }
                Text("Juz \(juz) sealed")
                    .font(QPTheme.serif(25))
                    .foregroundColor(QPTheme.ink)
                Text("\(QuranMap.juzOpenings[juz - 1]) is behind you — \(30 - juz) part\(30 - juz == 1 ? "" : "s") remain. The seal joins your wall in the Mushaf tab.")
