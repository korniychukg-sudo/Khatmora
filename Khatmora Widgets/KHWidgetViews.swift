...
struct PortionWidgetView: View {
    ...
    private var inline: some View {
        Group {
            if !snapshot.hasPlan {
                Text("Khatmora: crea un jatm")
            } else if snapshot.todayDone {
                Text("Porción lista · yuz \(snapshot.juz)")
            } else {
                Text("Quedan \(max(0, snapshot.todayGoal - snapshot.todayRead)) páginas · pág. \(snapshot.position + 1)")
            }
        }
    ...
    private var rectangular: some View {
        HStack(spacing: 8) {
            ...
            VStack(alignment: .leading, spacing: 1) {
                Text(snapshot.hasPlan ? "Hoy: \(snapshot.todayRead) de \(snapshot.todayGoal)" : "Khatmora")
                    .font(.system(size: 13, weight: .semibold))
                Text(snapshot.hasPlan ? "Yuz \(snapshot.juz) · \(snapshot.surahName)" : "Configura un jatm")
                    .font(.system(size: 11))
                    .opacity(0.8)
            }
            Spacer(minLength: 0)
    ...
    private var small: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            if snapshot.hasPlan {
                ...
                Text(snapshot.todayDone ? "Porción lista" : "páginas leídas")
                ...
                Text("Yuz \(snapshot.juz) · \(snapshot.surahName)")
        ...
        HStack(spacing: 14) {
            VStack(alignment: .leading, spacing: 5) {
                Text("PORCIÓN DE HOY")
                    ...
                    .foregroundColor(WPal.gold)
                if snapshot.hasPlan {
                    Text("\(snapshot.todayRead) de \(snapshot.todayGoal) páginas")
                    ...
                    Text(snapshot.portionLine)
                        ...
                        .foregroundColor(snapshot.todayDone ? WPal.sage : WPal.inkSoft)
                    Spacer(minLength: 0)
                    Text("Yuz \(snapshot.juz) · \(snapshot.juzOpening) · \(snapshot.surahName)")
                        .font(.system(size: 10, weight: .medium))
                        .foregroundColor(WPal.inkSoft)
                        .lineLimit(1)
                    ...
                    Text("No hay ningún jatm en curso")
                        ...
                        .foregroundColor(WPal.ink)
                    Text("Abre Khatmora para comenzar uno.")
                        .font(.system(size: 12))
                        .foregroundColor(WPal.inkSoft)
                    Spacer(minLength: 0)
                }
...
struct KhatmRingWidgetView: View {
    ...
    private var rectangular: some View {
        VStack(alignment: .leading, spacing: 2) {
            Text("Jatm · \(Int(snapshot.percent * 100))%")
                .font(.system(size: 13, weight: .semibold))
            ProgressView(value: min(1, snapshot.percent))
            Text("Página \(snapshot.position) de 604 · racha de \(snapshot.streak) días")
                .font(.system(size: 11))
                .opacity(0.8)
        }
        .containerBackground(for: .widget) { Color.clear }
    ...
    private var small: some View {
        VStack(spacing: 6) {
            ...
            Text(snapshot.hasPlan ? "Yuz \(snapshot.juz) · \(snapshot.surahName)" : "No hay ningún jatm en curso")
                .font(.system(size: 10, weight: .medium))
                .foregroundColor(WPal.inkSoft)
                .lineLimit(1)
        }
